...
func TestService_AddResources_ErrorOnRawManifestResource(t *testing.T) {
	service := componentconstructor.NewService()
	constructor := component.NewConstructor(testModuleName, testModuleVersion)
	moduleConfig := &contentprovider.ModuleConfig{
		Name:    testModuleName,
	...
	invalidPath := "invalid\x00path/manifest.yaml"
	resourcePaths := types.NewResourcePaths("", invalidPath, testModuleTemplatePath)

	err := service.AddResources(constructor, moduleConfig, resourcePaths)

	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to create raw manifest resource")
}

func TestService_AddResources_ErrorOnDefaultCRResource(t *testing.T) {
	service := componentconstructor.NewService()
	constructor := component.NewConstructor(testModuleName, testModuleVersion)
	moduleConfig := &contentprovider.ModuleConfig{
		Name:    testModuleName,
	...
	invalidPath := "invalid\x00path/defaultcr.yaml"
	resourcePaths := types.NewResourcePaths(invalidPath, testManifestPath, testModuleTemplatePath)

	err := service.AddResources(constructor, moduleConfig, resourcePaths)

	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to create default CR resource")
}

func TestService_AddResources_ErrorOnModuleTemplateResource(t *testing.T) {
	service := componentconstructor.NewService()
	constructor := component.NewConstructor(testModuleName, testModuleVersion)
	moduleConfig := &contentprovider.ModuleConfig{
		Name:    testModuleName,
	...
	invalidPath := "invalid\x00path/moduletemplate.yaml"
	resourcePaths := types.NewResourcePaths("", testManifestPath, invalidPath)

	err := service.AddResources(constructor, moduleConfig, resourcePaths)

	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to create moduletemplate resource")
}

func TestService_AddResources_EmptyPaths(t *testing.T) {
	service := componentconstructor.NewService()
	constructor := component.NewConstructor(testModuleName, testModuleVersion)
	moduleConfig := &contentprovider.ModuleConfig{
		Name:    testModuleName,
	...
	resourcePaths := types.NewResourcePaths("", "", testModuleTemplatePath)
...
func TestService_AddResources_RelativePathsHandling(t *testing.T) {
	service := componentconstructor.NewService()
	constructor := component.NewConstructor(testModuleName, testModuleVersion)
	moduleConfig := &contentprovider.ModuleConfig{
		Name:    testModuleName,
	...
	resourcePaths := types.NewResourcePaths(
		"./test/defaultcr.yaml",
		"../manifest.yaml",
		"../../moduletemplate.yaml",
	)
	...
	require.NoError(t, err)
	require.Len(t, constructor.Components[0].Resources, 4) // includes default CR

	for _, resource := range constructor.Components[0].Resources {
		if resource.Input != nil && resource.Input.Path != "" {
			require.True(t, filepath.IsAbs(resource.Input.Path),
				"Resource %s path should be absolute: %s", resource.Name, resource.Input.Path)
		}
	}
}
