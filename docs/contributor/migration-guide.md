...
| *n/a*                                          | `manager`                | Defines the module’s manager (name, group, version, kind, namespace)                      |
...
The `module-releases.yaml` file `ModuleReleaseMeta` Custom Resource (CR) defines how logical release channels (e.g., `regular`, `fast`, `experimental`, `dev`) map to specific module versions.  
This configuration is to ensure the subscription to a given channel receive the appropriate version of your module.

The `ModuleReleaseMeta` CR includes the following fields:

- **`spec.moduleName`**:  
  Specifies the name of the module.

- **`spec.beta`**:  
  A boolean flag indicating whether the release is a beta version. When set to `true`, it signifies that the release is in a beta state, potentially less stable and intended for testing purposes.

- **`spec.internal`**:  
  A boolean flag indicating whether the release is internal. When set to `true`, the release is intended for internal use only and is not exposed to external clients.

- **`spec.channels`**:  
  A list of channel-to-version mappings, where each entry specifies a `channel` and its corresponding `version`.
...
| **spec.manager.name**                                               | _omitted_                                            | `<module-manager-name>`                                     |
| **spec.manager.namespace**                                          | _omitted_                                            | `kyma-system`                                               |
| **spec.manager.group**                                              | _omitted_                                            | `apps`                                                      |
| **spec.manager.version**                                            | _omitted_                                            | `v1`                                                        |
